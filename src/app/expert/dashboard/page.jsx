...
import {
  ...
  PAYMENT_METHOD_CCP, PAYMENT_METHOD_BARIDIMOB, PAYMENT_METHOD_LABELS, DAYS_PATTERN_LABELS,
} from "@/lib/utils";
        ...
        <div className="bg-white rounded-2xl p-5 mb-4" style={{ border: `1px solid ${C_SAGE_LINE}` }}>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-0.5">
              ...
              {booking.has_rotating_sections && <SummaryRow label="أقسام دوّرة" value="نعم ✓" />}
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {Object.entries(booking.sections_breakdown).filter(([,v]) => v).map(([k, v]) => (
                      <div key={k} className="flex items-center justify-between py-1 px-2 rounded-lg" style={{ backgroundColor: "#F5F6F0" }}>
                        ...
                        <span className="font-bold text-xs" style={{ color: C_INK }}>{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="mb-0">
            ...
            <div className="space-y-0.5">
              ...
              <SummaryRow label="تاريخ الحجز" value={formatDate(booking.created_at)} />
          ...
          {booking.map_image_url && (
            <div className="mt-4">
              ...
              <img src={booking.map_image_url} alt="خريطة الأقسام" className="w-full rounded-xl border" style={{ borderColor: "#DCE2D6", maxHeight: 200, objectFit: "contain" }} />
            </div>
          )}
